...
from scripts import reference_count_execution as count_execution
...
def test_campaign_hashes_cover_every_behavior_owning_split_module():
    required = {
        "scripts/reference_acquisition_artifacts.py",
        "scripts/reference_artifact_run_validation.py",
        "scripts/reference_count_execution.py",
        "scripts/reference_preparation_artifacts.py",
        "scripts/reference_window_fetch.py",
        "scripts/reference_window_read.py",
    }

    assert required <= set(runtime.CAMPAIGN_SOURCE_FILES)
...
def _install_acquisition_adapters(
    ...
    *,
    ...
    monkeypatch.setattr(acquisition_artifacts, "load_source_header", replay_header)
...
def _install_preparation_adapters(
    ...
    *,
    ...
    if not variants:
        ...
        monkeypatch.setattr(count_execution, "native_called_totals", forbidden)
        monkeypatch.setattr(count_execution, "query_native_tokens", forbidden)
        return
    ...
    monkeypatch.setattr(count_execution, "native_called_totals", native_counts)
    monkeypatch.setattr(count_execution, "query_native_tokens", native_tokens)
...
def test_one_stage_failure_withdraws_every_stage_summary(tmp_path, monkeypatch):
    ...
    native_counts = count_execution.native_called_totals
    ...
    monkeypatch.setattr(count_execution, "native_called_totals", fail_paper)
