...
def _synthetic_artifact_adapters(monkeypatch):
    ...
    def qualify_synthetic(root, hashes, expected=None):
        for field, relative in artifact_io._COHORT_PATHS.items():
    ...
    monkeypatch.setattr(artifact_io, "_qualify_cohort_files", qualify_synthetic)
    monkeypatch.setattr(
        artifact_io,
        "parse_header",
    ...
    def load_source_header(
        ...
        return raw, artifact_io.parse_header(
            raw,
    ...
    monkeypatch.setattr(artifact_io, "load_source_header", load_source_header)
...
def test_acquisition_writer_removes_its_manifest_after_postwrite_source_change(
    ...
    manifest, preflight = _complete_acquisition_tree(tmp_path)
    original = artifact_io._check_provenance
    calls = 0
    ...
    monkeypatch.setattr(artifact_io, "_check_provenance", changed)
...
def test_preparation_writer_removes_its_manifest_after_failed_readback(
    ...
    write_acquisition_manifest(acquisition_root, acquisition, preflight=preflight)
    preparation = _complete_empty_preparation_tree(counts_root, acquisition_root)
    original = artifact_io._write_exclusive

    def changed(path, raw):
        original(path, raw)
        path.write_bytes(b"{}\n")

    monkeypatch.setattr(artifact_io, "_write_exclusive", changed)
