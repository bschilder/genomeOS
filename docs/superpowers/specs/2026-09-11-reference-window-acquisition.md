...
**Status:** all six software implementation tasks are complete and independently reviewed for #255, advancing the owner-authorized #189 global AF program WP0/WP1. Real source acquisition and count preparation have not run; they remain in the controller-only reviewed phase. Implements Atlas §§4–8,12. Companion: `docs/superpowers/plans/2026-09-11-reference-window-acquisition.md` and software evidence: `docs/research/reference-window-acquisition-2026-09-11.md`.
...
`stage_sparse` returns `VerifiedSource`, not None. It consumes the actual `RangeReceipt` tuple, verifies it exactly covers the source's reviewed merged plan, requires every receipt verified with a retained file, rehashes every complete range file and the retained index, writes sparse extents, and reads back/hashes those extents against the range files. `VerifiedSource.ranges` then lists the **actually acquired** source offsets and file hashes. It is not created from planned coordinates alone. The source pair/generations, exact logical size, index identity and actual extents must all match the reviewed source plan. Because the schema has no independently replayable sparse-stage failure receipt, a sparse construction/allocation failure aborts before the final manifest instead of publishing an unsupported refusal claim; verified transport artifacts remain on disk for diagnosis.
...
AcquisitionReviewBundle(schema_version: Literal['reference_acquisition_review_bundle_v1'],
                        preflight: ReviewReceipt, implementation_revision: str,
                        implementation_sha256: HashEntries, review_locator: str,
                        review_sha256: Digest, status: Literal['accepted'])
...
AcquisitionInputs(window_manifest: ArtifactRef, windows: ArtifactRef,
                  ...
                  review: ArtifactRef[AcquisitionReviewBundle], cohort: CohortInputHashes)
...
NativeRunReceipt(operation: Literal['extract_bcf','query_keys','select_cohort','fill_tags',
                                   'query_samples','query_tokens','query_totals'],
...
| `genomeos/validation/reference_count_types.py` | Dependency-light immutable reference-count records and row invariants. |
...
| `scripts/reference_window_io.py` | Stable facade for bounded window acquisition and native reads. |
| `scripts/reference_window_fetch.py` | Bounded transport, metadata and sparse-source staging. |
| `scripts/reference_window_read.py` | Verified header/original-record and native artifact readers. |
| `scripts/reference_native_io.py` | Bounded native subprocess execution, BCF extraction and key validation. |
| `scripts/reference_runtime.py` | Version, executable/library/plugin and Cloud SDK source identity. |
| `scripts/reference_artifact_io.py` | Bounded immutable path, hash, read and exclusive-write primitives. |
| `scripts/reference_artifact_inventory.py` | Exact role-derived phase inventories and canonical ledger checks. |
| `scripts/reference_cohort_artifacts.py` | Retained cohort-file qualification and hash binding. |
| `scripts/reference_window_artifacts.py` | Stable artifact validation/writer facade. |
| `scripts/reference_artifact_run_validation.py` | Shared native/transport run-receipt validation. |
| `scripts/reference_acquisition_artifacts.py` | Exclusive acquisition writer and deep acquisition validation. |
| `scripts/reference_preparation_artifacts.py` | Exclusive preparation writer and deep preparation validation. |
| `scripts/reference_count_artifacts.py` | Disk-backed count-table, QC and dependency artifact validation. |
| `scripts/reference_count_replay.py` | Streaming replay of original/native evidence into expected counts and refusal causes. |
| `scripts/reference_count_rows.py` | Strict admitted and refused-fragment count-row streams. |
| `scripts/reference_native_stage.py` | Per-stage native control orchestration and normalized refusals. |
| `scripts/reference_count_execution.py` | One-pass paired-cohort count execution and native cross-checks. |
| `scripts/reference_preparation_outputs.py` | Immutable preparation fragments, ledgers and external merges. |
