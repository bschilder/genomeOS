...
Before byte acquisition, validate the whole input manifest and freeze parser policy/limits; check all declared TBI sizes and planned fixed overhead. Read every pinned VCF/TBI object's metadata through `python scripts/gcloud_repo.py run storage objects describe 'gs://…#GENERATION' --format=json`; match generation, size and both checksums. Use anonymous credentials-disabled, file-logging-disabled wrapper invocations with argument arrays and no shell. Never display environment values or inspect auth stores.
