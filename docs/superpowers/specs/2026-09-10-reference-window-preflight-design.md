...
| `indexes/chrN.tbi` | Exact complete TBI bytes for each integrity-verified, successfully parsed source, including sources with `no_index_chunks` windows. Deterministic chr1–chr22 basenames; SHA-256 must equal that source receipt. No file for refused sources. |
...
Retain each complete, checksum-verified and successfully parsed TBI as `indexes/chrN.tbi` in the exclusive output directory before writing the completion `preflight.json`. This supplies the same native index bytes to the subsequent acquisition task without a second body request; it adds no request, budget allowance or schema field. Preserve verified indexes from independent successful sources when another source refuses. Failed, partial, checksum-mismatched or parser-invalid bodies never enter this directory. Future consumers verify retained bytes against the source receipt and source identities before use; pathname alone is not identity.
