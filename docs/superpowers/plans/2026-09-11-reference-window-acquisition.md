...
- [ ] **RED: hand cohort, collisions and reordering.**
...
- [ ] **RED: native sample identity survives independent reordering.**
...
- [ ] **GREEN/gates/review:** both focused files, smoke, module size, privacy; inspect staged files; commit `feat: preserve original reference genotype count semantics; advance #255`. Review specifically checks old parser decision ordering and explicit validation coverage, not just numeric happy paths.
...
- [ ] **RED: identity/orientation/AN=0 projection.**
...
- [ ] **GREEN/gates/review:** both focused files, smoke, module size/privacy, staged-path inspection; commit `feat: prepare traceable reference window count artifacts; advance #255`.
...
- [ ] **RED: CLI gate and failure ordering.**
...
- [ ] **Run RED:** `PYTHONPATH=. python -m pytest tests/test_reference_window_clis.py -q`.
- [ ] **GREEN:** implement CLI argument parsing with all design §7 required flags and no permissive options. Check imported module paths/revision/hashes and tool versions before VCF requests; check all inputs/review/indexes first, then acquire, qualify source header/sample order, recover raw/native windows, and write the final acquisition ledger. Offline preparation validates full acquisition, requalifies frozen cohort inputs, projects original tokens, runs native called controls per stage/window, writes private artifacts and validates them. No network in preparation. On local invalid input before an output exists, exit with no directory; on begun work failure, preserve terminal manifest-last accounting.
- [ ] **Wire exact phase interfaces, without running real acquisition in the task.** Acquisition composition loads the reviewed preflight once, produces each actual RangeReceipt, then calls `stage_sparse` to obtain VerifiedSource. It passes that receipt and the matching SourceBytePlan to raw/native readers. Assemble AcquisitionSourceReceipt and AcquisitionWindowReceipt independently; call `write_acquisition_manifest(out, manifest, preflight=preflight)` last and then `validate_acquisition(out)`. Preparation starts with `parent = validate_acquisition(acquisition_root)` and explicitly requires parent.complete before any count work. For each cohort call `native_called_totals(parent_window.native_bcf, cohort_file, acquisition_root=acquisition_root, artifact_root=out, bcftools=bcftools, output_prefix=prefix)` and retain its NativeCountFiles in the stage.native_control field, then `read_native_totals(control, artifact_root=out)`; call `query_native_tokens(control.selected_bcf, artifact_root=out, bcftools=bcftools, output_prefix=prefix)` after a complete control, retain it in stage.native_tokens, and feed each `NativeVariantTokens` to the identity-joining checker. Create a separate PreparationWindowReceipt with exactly two PreparationStageReceipts for every expected window. Finalize with `write_preparation_manifest(out, manifest, acquisition_root=acquisition_root)` and `validate_preparation(out, acquisition_root=acquisition_root)`. The phase codecs/writers use the exact types and sidecar columns defined globally, copying the exact private cohort inputs to their fixed local evidence paths before any final writer; Task 6 adds no schema or writer interface. Known complete intermediate files survive a refused result; phase complete flags never infer approval.
- [ ] **Run full software gate:**
...
- [ ] **Independent pre-retrieval review:** reviewer checks all source/count/transport invariants, known control limits and exact #254 preflight/source hashes. Controller records the hash-bound accepted receipt; do not issue a self-approval. Verify #254's added honest transport policy is consumed strictly. Confirm actual planned bytes≤cap and retained 22 indexes present. No owner reauthorization is required for this already authorized campaign, but the specified independent review must actually pass.
- [ ] **Run real acquisition and offline count preparation** using design §7 exact commands with caller-resolved private paths and new output directories. No speculative retry or fitting. Retain partial/failure artifacts and report window outcomes if a source/format/budget/runtime condition refuses. If a code fix is needed, redo applicable checks/review on new source and use a new run; preserve the original attempt. Document actual body bytes separately from planned total and SDK metadata uncertainty.
- [ ] **Independent real-evidence audit:** verify all 66 ledger states; exact 22 sources/generations/range/index receipts; raw/header/native/sample hashes and source order joins; exact 4,117/4,094 and 80 populations; every retained variant's native called AC/AN match in both stages; all 4 keysets, AN=0 rows, sequential reasons and source/window/dependency sidecars; content validators; read-only preservation of original pilot inputs. Report exactly which population/quality checks are hand-derived versus independently native-validated. If any window refused, report incomplete research preparation and do not close #255 as completed.
- [ ] **Final documentation and PR:** write aggregate research note with commands/revisions/check counts, source provenance, all-window state histogram, four-table row/unavailable totals, native match totals and transport/independence/geography limitations. No participant IDs, per-variant/source count dataset or local private paths in public docs. Rerun smoke after the last code/config/runtime change and applicable focused tests after fixes; full CI before PR. Before commit/push run privacy gate and inspect staged paths. Commit the final completed work with `feat: prepare frozen reference window development counts; closes #255`; push dedicated branch and open one PR explicitly `Closes #255` and “advances #189; implements Atlas §§4–8,12”. State that data preparation grants no calibration/model/publication claim.
