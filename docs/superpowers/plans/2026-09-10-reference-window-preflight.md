...
The receipt `*_attempts` fields count adapter wrapper invocations, not HTTP attempts internal to
gcloud. Fixed preflight policy records `request_attempt_accounting="wrapper_invocations"`,
`request_attempts_per_object=1`, `storage_body_max_retries=0` and
...
- [ ] **RED: adapter confinement.** Fake `Popen` streams supply pinned metadata and synthetic index bodies, preserving byte boundaries. Assert argv contains the repository wrapper and exact `#generation`, anonymous/file-log-disable environment keys, forced `CLOUDSDK_STORAGE_MAX_RETRIES=0` even when the inherited value is nonzero, and no VCF `storage cat` invocation. Test metadata generation/size/checksum mismatch; unavailable pinned generation; partial/oversized body; checksum mismatch; 120-second timeout; 1 MiB metadata/16 MiB compressed cap; malformed manifest causes zero process calls. Verify 22×3 retained ledger rows after one index failure, nonzero exit, incomplete/null total, and successful independent chromosomes. Count metadata adapter invocations separately from index body adapter invocations; do not interpret these receipt counters as internal HTTP attempts.
- [ ] **GREEN: implement bounded adapter/CLI.** Require `--manifest`, `--windows`, `--out`; verify input bytes and strict manifest before any network. Use argument arrays, `shell=False`, wrapper `storage objects describe --raw --format=json` for pinned VCF and TBI identities (raw API `generation`, decimal-string `size`, `md5Hash`, `crc32c`), then `storage cat` only for verified pinned TBI. Enforce streaming size/time bounds; no unbounded `capture_output` for public body reads. One metadata adapter invocation per object and at most one index-body adapter invocation per pair; no adapter retry or fallback. Force `CLOUDSDK_STORAGE_MAX_RETRIES=0` to disable body retries. Installed Google Cloud SDK 574.0.0 source plus an offline injected-failure control showed that zero permits the initial body attempt without its retry callback. Record metadata HTTP attempts as unobserved: `storage cat` performs an internal metadata lookup, the metadata API separately defaults to five retries, and any internal activity remains bounded by the wrapper invocation timeout/output cap. Build fixed-reason receipts, preserve body SHA-256 only after integrity checks, continue independent pairs, emit canonical `preflight.json` last and exit nonzero for any refusal or excess budget. Persist each complete integrity-verified, successfully parsed TBI as `indexes/chrN.tbi` inside the exclusive output, with exact bytes matching its receipt SHA-256; include `no_index_chunks` sources and retain good indexes even if another source fails. No cache file for refused/partial/invalid bodies; write preflight.json last. Test exact successful index bytes/hashes and file absence for each refusal, including an incomplete multi-source run. Keep failed partial bodies out of successful caches. Hash imported modules/CLI/wrapper and record native/wrapper version evidence separately from scientific timing.
