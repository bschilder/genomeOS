#!/usr/bin/env python3
"""Migrate published P2 surfaces into the P4 serving catalog (design §5, §6, §10).

The P2 publisher writes one immutable directory per entity. P4 needs one catalog that can resolve
those objects without guessing labels, measurement semantics, eligibility, or versions. This
command requires those decisions in a reviewed metadata file, derives only mechanical H3 fields,
and writes the catalog manifest last so a partial build can never look published.
"""
...
from genomeos.artifacts import ArtifactFile, ArtifactManifest, VariantEntry
